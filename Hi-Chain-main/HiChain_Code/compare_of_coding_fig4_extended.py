...
BETA_FIXED = 0.5   # subplots 1-2: fixed beta
ALPHA_FIXED = 5    # subplots 3-4: fixed alpha

# figure output folder
FIGURE_OUTPUT_FOLDER = "encoding_figures"
...
def test_adaptive_rs_encoding():
    ...
    # subplots 1-2: fixed beta, sweep ALPHAS
    # subplots 3-4: fixed alpha, sweep BETAS
    alpha_beta_combinations = []
    for alpha in ALPHAS:
        alpha_beta_combinations.append((alpha, BETA_FIXED))
    for beta in BETAS:
        if (ALPHA_FIXED, beta) not in alpha_beta_combinations:
            alpha_beta_combinations.append((ALPHA_FIXED, beta))
    ...
    print(f"alpha取值: {ALPHAS}")
    print(f"beta取值: {BETAS}")
...
def plot_results(df, n_nodes):
    """Plot and save figures locally."""

    if not os.path.exists(FIGURE_OUTPUT_FOLDER):
        os.makedirs(FIGURE_OUTPUT_FOLDER)
        print(f"✓ 创建图像文件夹: {FIGURE_OUTPUT_FOLDER}")

    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")

    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(20, 16))
    fig.suptitle('Adaptive RS Encoding Parameter Comparison', fontsize=24, fontweight='bold')

    colors = plt.cm.tab10(np.linspace(0, 0.5, len(ALPHAS)))

    # 1. access frequency vs k (fixed beta)
    for i, alpha in enumerate(ALPHAS):
        ...
        if not subset.empty:
            ax1.plot(subset['access_frequency'], subset['k_value'],
                     ...
                     label=f'$\\alpha$={alpha}, $\\beta$={BETA_FIXED}')
            ax1.scatter(subset['access_frequency'], subset['k_value'],
                        c=[colors[i]], s=30, alpha=0.6)

    ax1.set_xlabel('Access Frequency')
    ax1.set_ylabel('k Value (Required Fragments)')
    ax1.set_title(f'Access Frequency vs k Value ($\\beta$={BETA_FIXED} fixed)')
    ax1.grid(True, alpha=0.3)
    ax1.legend()

    # 2. access frequency vs redundancy (fixed beta)
    for i, alpha in enumerate(ALPHAS):
        mask = (df['alpha'] == alpha) & (df['beta'] == BETA_FIXED)
        subset = df[mask]
        if not subset.empty:
    ...
    ax2.set_xlabel('Access Frequency')
    ax2.set_ylabel('Redundancy')
    ax2.set_title(f'Access Frequency vs Redundancy ($\\beta$={BETA_FIXED} fixed)')
    ax2.grid(True, alpha=0.3)
    ax2.legend()

    colors_beta = plt.cm.Set2(np.linspace(0, 1, len(BETAS)))

    # 3. access frequency vs k (fixed alpha)
    for i, beta in enumerate(BETAS):
        mask = (df['alpha'] == ALPHA_FIXED) & (df['beta'] == beta)
        subset = df[mask]
        if not subset.empty:
            ax3.plot(subset['access_frequency'], subset['k_value'],
                     color=colors_beta[i], linewidth=2.5, alpha=0.8,
                     label=f'$\\alpha$={ALPHA_FIXED}, $\\beta$={beta}')
            ax3.scatter(subset['access_frequency'], subset['k_value'],
                        c=[colors_beta[i]], s=30, alpha=0.6)

    ax3.set_xlabel('Access Frequency')
    ax3.set_ylabel('k Value (Required Fragments)')
    ax3.set_title(f'Access Frequency vs k Value ($\\alpha$={ALPHA_FIXED} fixed)')
    ax3.grid(True, alpha=0.3)
    ax3.legend()

    # 4. access frequency vs redundancy (fixed alpha)
    for i, beta in enumerate(BETAS):
        mask = (df['alpha'] == ALPHA_FIXED) & (df['beta'] == beta)
        subset = df[mask]
        if not subset.empty:
    ...
    ax4.set_xlabel('Access Frequency')
    ax4.set_ylabel('Redundancy')
    ax4.set_title(f'Access Frequency vs Redundancy ($\\alpha$={ALPHA_FIXED} fixed)')
    ...
    combined_path = os.path.join(
        FIGURE_OUTPUT_FOLDER, f'adaptive_rs_comparison_{timestamp}.png'
    )
    fig.savefig(combined_path, dpi=300, bbox_inches='tight')
    print(f"✓ 综合对比图已保存到: {os.path.abspath(combined_path)}")

    # save individual subplots
    subplot_configs = [
        (ax1, f'k_vs_frequency_beta{BETA_FIXED}'),
        (ax2, f'redundancy_vs_frequency_beta{BETA_FIXED}'),
        (ax3, f'k_vs_frequency_alpha{ALPHA_FIXED}'),
        (ax4, f'redundancy_vs_frequency_alpha{ALPHA_FIXED}'),
    ]
    for ax, name in subplot_configs:
        fig_single = plt.figure(figsize=(12, 9))
        ax_single = fig_single.add_subplot(111)
        for line in ax.get_lines():
            ax_single.plot(
                line.get_xdata(), line.get_ydata(),
                color=line.get_color(), linewidth=2.5, alpha=0.8,
        ...
        for collection in ax.collections:
            ...
            if len(offsets) > 0:
                ax_single.scatter(
                    offsets[:, 0], offsets[:, 1],
                    c=collection.get_facecolors(), s=30, alpha=0.6
                )
        ...
        single_path = os.path.join(
            FIGURE_OUTPUT_FOLDER, f'{name}_{timestamp}.png'
        )
        fig_single.savefig(single_path, dpi=300, bbox_inches='tight')
        plt.close(fig_single)
        print(f"✓ 子图已保存到: {os.path.abspath(single_path)}")
